    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            if cur_pos == min_prompt_len:
                ...
                initial_tokens = [self.tokenizer.decode([x].tolist()) for x in initial_tokens[0]]
                print('initial_tokens:', initial_tokens)
...
def sample_top_p(probs, p):
    ...
    """
    probs_sort, probs_idx = torch.sort(probs, dim=-1, descending=True)
    # probs_sort.shape: torch.Size([6, 32000])
    # probs_idx.shape:  torch.Size([6, 32000])
    probs_sum = torch.cumsum(probs_sort, dim=-1)
    mask = probs_sum - probs_sort > p
    probs_sort[mask] = 0.0
    probs_sort.div_(probs_sort.sum(dim=-1, keepdim=True))
    next_token = torch.multinomial(probs_sort, num_samples=1) # next_token.shape: torch.Size([6, 1])
    next_token = torch.gather(probs_idx, -1, next_token) # next_token.shape: torch.Size([6, 1])
    return next_token
